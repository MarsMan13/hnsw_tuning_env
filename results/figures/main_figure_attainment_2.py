...
# --- Styles per solution ---
SOL_STYLES = {
    "our_solution":  {"c": "#d62728", "marker": "o", "ls": "-",  "lw": 2.0, "zorder": 10, "label": "CHAT (Ours)"},
...
# Base target levels (discrete)
ALPHAS_BASE = [0.7, 0.8, 0.9, 0.925, 0.95, 0.975, 0.99]
TAIL_START = 0.99  # start of "dense" region up to 1.0
...
def build_attainment_points(
    ...
    - For alphas_base (<=0.99): first hit times (y=alpha)
    ...
    for alpha in alphas_base:
        ...
        if found_t is not None:
            plot_xs.append(max(found_t, 1.0))
            plot_ys.append(alpha)

    # 4) tail: every improvement after reaching tail_start
    ...
    if start_idx is not None:
        ...
        for t, b in zip(times[start_idx:], best_perfs[start_idx:]):
            ...
            # Avoid duplicating the base 0.99 point exactly
            if last_base_t is not None and last_base_y is not None:
                if abs(safe_t - last_base_t) < 1e-12 and abs(ratio - last_base_y) < 1e-12:
                    continue
...
def plot_attainment_on_ax(
    ...
    for sol in SOL_ORDER:
        ...
        plot_xs, plot_ys = build_attainment_points(
            data=data,
    ...
    # --- Formatting ---
    ax.set_xscale("log")
    ax.set_xlim(1.0, tuning_budget * 1.05)
    ...
    if show_ylabel:
        ax.set_yticklabels([f"{int(a*100)}%" for a in yticks], fontsize=9)
    else:
        ax.set_yticklabels([])
...
def main():
    ...
    for sol in SOL_ORDER:
        s = SOL_STYLES[sol]
        legend_elements.append(Line2D([0], [0], color=s["c"], marker=s["marker"], ls=s["ls"], lw=s["lw"], label=s["label"]))
