...
def build_attainment_points(
    ...
    hit_to_alpha = {}  # time -> max alpha reached at that time
    for alpha in alphas_calc:
        ...
        for t, b in zip(times, bests):
            if b >= target:
                ...
                break
        if hit is not None:
            prev = hit_to_alpha.get(hit, -1.0)
            if alpha > prev:
                hit_to_alpha[hit] = alpha

    plot_points = sorted(hit_to_alpha.items(), key=lambda x: x[0])
    plot_xs = [t for t, _ in plot_points]
    plot_ys = [a for _, a in plot_points]

    # 2) Tail points after tail_start: add only if time not already present
    ...
    existing_times = set(plot_xs)
    if start_idx is not None:
        for t, b in zip(times[start_idx:], bests[start_idx:]):
            ratio = min(float(b / oracle_best), 1.0)

            # If an alpha point already exists at the same time, keep the higher y
            if t in existing_times:
                idx = plot_xs.index(t)
                if ratio > plot_ys[idx]:
                    plot_ys[idx] = ratio
                continue
    ...
    # Ensure the curve always starts from (0, 0%)
    if not plot_xs:
        plot_xs = [0.0]
        plot_ys = [0.0]
    else:
        if plot_xs[0] > 1e-12:
            plot_xs.insert(0, 0.0)
            plot_ys.insert(0, 0.0)
        else:
            plot_ys[0] = 0.0
