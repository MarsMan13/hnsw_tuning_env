...
def _3d_performance_plotter(impl, dataset, recall_min, qps_min, sampling_count):
    ...
    try:
        ...
        results = load_search_results(
            solution, filename, seed=MOCK_SEED, sampling_count=sampling_count
        )
        
        if not results:
            print(f"[PID {pid}] Skipping {filename}: No results found.")
            return

        # 데이터 파싱: 중복되는 (M, efc) 조합 중 최고 성능만 추출
        M_efc_to_perf = dict()
        ...
        for res in results:
            M, efc, efs = res[0]
            target_perf = res[1][1] if qps_min is not None else res[1][2] 
            
            is_feasible = False
            if qps_min is not None and res[1][2] >= qps_min: 
                is_feasible = True
            if recall_min is not None and res[1][1] >= recall_min: 
                is_feasible = True
