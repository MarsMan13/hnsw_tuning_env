...
class HyperparameterTuner:
    ...
    def __init__(
        ...
        self.build_time_estimator = MockBuildTimeEstimator(threshold=build_time_budget)
        self.index_size_estimator = MockIndexSizeEstimator(N=N, d=d, threshold=index_size_budget)
    ...
    def _evaluate_hp(self, m: int, efc: int) -> float:
        ...
        is_valid_build_time = self.build_time_estimator.update(efC=efc, M=m, build_time=build_time)
        is_valid_index_size = self.index_size_estimator.update(efC=efc, M=m, index_size=index_size)
        if not is_valid_build_time or not is_valid_index_size:
            return -float('inf')
