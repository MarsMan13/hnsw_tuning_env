    ...
    try:
        while 3 < M_right - M_left:
            ...
            for M in [M_mid1, M_mid2]:
                ...
                efS_getters[M] = EfSGetter()
                efS_getter = efS_getters[M]
                efC_iter_limit = math.ceil(math.log(EFC_MAX - EFC_MIN, 3))   #! TODO
                efC_count = 0
                max_perf_of_M = 0.0
                while 3 < efC_right - efC_left and efC_count < efC_iter_limit:
                    ...
                    print(f"\t\t[{M}] - efC : {efC_mid1} -> {get_perf(perf_mid1)}, efC : {efC_mid2} -> {get_perf(perf_mid2)} -> {max_perf_of_M}")
                efC_getter.put(M, efC_left * LOWER_MUL_FACTOR, efC_right * UPPER_MUL_FACTOR)
                M_to_perf.append((M, max_perf_of_M, efC_left, efC_right))
                print(f"\tM : {M} -> {max_perf_of_M}")
        ...
        for M in [M_left, M_right]:
            if M in [m for m, *_ in M_to_perf]:
                continue
            # efC_left, efC_right = efC_getter.get(M)
            efC_left, efC_right = EFC_MIN, EFC_MAX
            efS_getters[M] = EfSGetter()
            efS_getter = efS_getters[M]
            efC_iter_limit = math.ceil(math.log(EFC_MAX - EFC_MIN, 3)) // 3
            efC_count = 0
            max_perf_of_M = 0.0
            while 3 < efC_right - efC_left and efC_count < efC_iter_limit:
                efC_count += 1
            ...
            perf_mid1 = get_max_perf(results, M_left, recall_min=recall_min, qps_min=qps_min)
            perf_mid2 = get_max_perf(results, M_right, recall_min=recall_min, qps_min=qps_min)
            print(f"M : {M_left} -> {perf_mid1}, M : {M_right} -> {perf_mid2}\n")
    except TimeoutError:
        print("Explration Time Out!")
    _M_to_perf = _M_to_perf[::-1]
    for M, perf, M_left, M_right in _M_to_perf:
        print(f"{M} {round(float(perf))} {M_left} {M_right}")
...
def exploitation_phase(results, ground_truth:GroundTruth, exploitation_budget, recall_min=None, qps_min=None):
    ...
    if not M_to_perf:
        ...
        return
    best_M_config = sorted(M_to_perf, key=lambda x: x[1], reverse=True)[0]
    sorted_M_configs = sorted(M_to_perf, key=lambda x: x[0])
    upper_M_config, lower_M_config = None, None
    sorted_M_to_perf = sorted(M_to_perf, key=lambda x: x[0])
    ...
    try:
        ...
        #     cur_M = M
        for M, efC in candidates:
            efS_getter = efS_getters.get(M, EfSGetter())
