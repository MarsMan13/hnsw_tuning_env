...
LOWER_MUL_FACTOR, UPPER_MUL_FACTOR = 0.5, 1.5
M_to_perf = []
searched_hp = set()
            ...
            for M in [M_mid1, M_mid2]:
                ...
                while 3 < efC_right - efC_left and efC_count < efC_iter_limit:
                    ...
                    if perf_mid1[1] <= perf_mid2[1]:
                        ...
                        efC_right = efC_mid2
                efCGetter.put(M, efC_left, efC_right)
...
def run(impl=IMPL, dataset=DATASET, recall_min=None, qps_min=None, tuning_budget=TUNING_BUDGET, sampling_count=None):
    ...
    results = []
    exploration_phase(results, gd, recall_min, qps_min, PHASE_THRESHOLD * tuning_budget)
    print(f"TT : {tuning_budget - gd.tuning_time} / {tuning_budget}")
    exploitation_phase(results, gd, (1.0 - PHASE_THRESHOLD) * tuning_budget, recall_min, qps_min)
