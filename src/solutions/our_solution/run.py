...
def _find_best_efc_for_m(M, ground_truth:GroundTruth, results, get_perf, recall_min, qps_min, exploration_budget, is_exploration=True):
    ...
    while 3 < efC_right - efC_left and efC_count < efC_iter_limit:
        ...
        # 1. Cache the performance values to avoid redundant function calls.
        #    This improves both efficiency and readability.
        perf1 = get_perf(perf_mid1)
        perf2 = get_perf(perf_mid2)
        if perf1 == 0 and perf2 == 0:
...
def _exploration_phase(results, ground_truth: GroundTruth, recall_min=None, qps_min=None, exploration_budget=TUNING_BUDGET):
    ...
    try:
        ...
        for M in [M_left, M_right]:
            if M in processed_M:
                continue
            max_perf_of_M = _find_best_efc_for_m(M, ground_truth, results, get_perf, recall_min, qps_min, exploration_budget)
            processed_M.add(M)
