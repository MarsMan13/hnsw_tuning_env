    ...
    for M in [16, 24, 32, 40, 48, 56, 64]:
        ...
        for efC in range(64, 1024+1, 8):
            ...
            if qps > opt_qps:
                ...
                opt_efC = efC
                opt_efS = efS
        print(f"[Optimal] ({M}, {opt_efC}, {opt_efS}) with QPS={opt_qps}")
        for efC in range(max(8, opt_efC - 128), min(1024, opt_efC + 128) + 1, 8):
            efS = gd.get_efS(M, efC, recall_min, method="binary")
            # efS = gd.get_efS(M, efC, recall_min, method="linear")
            recall, qps, total_time, build_time, index_size = gd.get(M=M, efC=efC, efS=efS, tracking_time=False)
            gd.tuning_time += 100.0
            print(f"\t({M}, {efC}, {efS}) - QPS: {round(qps.item())}, Recall: {round(recall.item(), 2)}")
