...
from src.solutions.vd_tuner.utils import *
...
def fast_non_dominated_sort(P): #* P : [[QPS, Recall], ...]
    def compare(p1, p2):
        D = len(p1)
        p1_dominate_p2 = True 
        p2_dominate_p1 = True
    ...
    #* END OF compare
    P_size = len(P)
    n = np.full(shape=P_size, fill_value=0) #* |[0, 0, ...]| = P_size
    #* n : [0, 0, ...] : how many points dominate p
    S = []  #* S : dominated points of each point
    f = []  #* f : all fronts
    rank = np.full(shape=P_size, fill_value=-1) #* |[-1, -1, ...]| = P_size
    #* rank : rank of each point, -1 means not assigned yet. 0 means first front, 1 means second front, etc.

    f_0 = []    #* First front
    for p in range(P_size):
        n_p = 0
        S_p = []    #* S_p : points dominated by p 
        for q in range(P_size):
            if p == q:
                continue
            cmp = compare(P[p], P[q])
            if cmp == 1:    #* p dominate q
                S_p.append(q)
            elif cmp == -1: #* q dominate p
                n_p += 1
        S.append(S_p)
        n[p] = n_p
        if n_p == 0:    #* There is no point dominating p
            rank[p] = 0
            f_0.append(p)
    f.append(f_0)
    i = 0
    while len(f[i]) != 0: 
        ...
        for p in f[i]:
            for q in S[p]:
                n[q] -= 1
                if n[q] == 0:   #* There is no point dominating q
                    rank[q] = i + 1 #* rank of q is i + 1
                    Q.append(q)
        i += 1
        f.append(Q)
    return rank, f  #* f : [[values of first front], [values of second front], ...] : all fronts


class EHVIBO:
    def __init__(self, knob_num, seed) -> None:
        self.knob_num = knob_num
        self.bounds = torch.tensor([[0.0] * self.knob_num, [1.0] * self.knob_num])  #* 2 x knob_num : [0.0, 1.0]
        self.seed = seed
        self.X_init = None
        self.Y_init = None
    ...
    def kernel_init(self,):
        ...
        self.covar_module = ScaleKernel(    #* Finally Created Kernel
...
class PollingBayesianOptimization:
    def __init__(self, env:MockEnv, seed=1206, threshold=None) -> None:
        self.env = env
        self.knob_num = len(env.names)  #* How many knobs(hyperparams) we have
        self.default_conf = self.env.default_conf() #* [normalized dfault param]
        self.vbo = EHVIBO(self.knob_num, seed=seed) #* Expected Hypervolume Improvement Bayesian Optimization
        self.seed = seed
        ...
        ##* X: index type -> [[normalized default params], [normalized params], ...]
        ##* Y: index type -> [[QPS, Recall, Tuning Time], [QPS, Recall, Tuning Time], ...]
    ...
    def init_sample(self):
        for k in self.remain_types:
            x = [self.default_conf[:]]  # [[normalized default params]]
            x[0][0] = self.env.knob_stand.scale_forward('index_type', k)
            # [[Normalized index type, normalized default params, ...]]
            y = self.env.get_state(x)
            # y : len(x) x 3(QPS, Recall, Tuning Time)
            self.X[k] = self.X[k] + x
    ...
    def reward_transform(self,):
        ...
        #* self.chose_ref_k : all Index Types
        for k, Y_k in self.Y.items():   #* k, Y_k : index type, [[QPS, Recall, Tuning Time], ...]
            Y_k_arr = np.array(Y_k)[:,:2]   #* Y_k_arr : [[QPS, Recall], ...]
            _, popu = fast_non_dominated_sort(Y_k_arr)

            fitness = -1 / (np.abs(Y_k_arr[:,0] / np.max(Y_k_arr[:,0]) - Y_k_arr[:,1] / np.max(Y_k_arr[:,1])) + 1e-6)
            ##* fitness = -1 / [abs(QPS/max(QPS) - Recall/max(Recall) + 1e-6]
            ##* |fitness| = len(Y_k_arr)
            fitness[popu[0]] = - fitness[popu[0]]   #* fitness[popu[0]] : the best point in the front

            chosen_idx = np.argmax(fitness) #* chosen_idx : the best point in the fitness
