...
def load_search_results(solution, filename):
    ...
    with open(load_path, "r") as f:
        ...
        for row in reader:
            ...
            index_size = int(row[8])
            results.append(((M, efC, efS), (T_record, recall, qps, total_time, build_time, index_size)))
    return results

def plot_timestamp(results, solution, filename, min_recall=0.95, accumulated=False, tuning_budget=TUNING_BUDGET, ylabel='QPS'):
    save_path = _save_path("timestamp", solution, filename)
    filtered_results = [
        (value[0], value[2])  # T_record, qps
        for _, value in results
        if value[1] >= min_recall and value[0] <= tuning_budget
    ]
    ...
    if accumulated:
        ...
        for i in range(len(results)):
            ...
            if qps > max_qps and recall >= min_recall and T_record <= tuning_budget:
    ...
    if not filtered_results:
        print(f"No results with recall >= {min_recall}")
        return
    timestamps, qps_values = zip(*filtered_results)
    plt.plot(timestamps, qps_values, marker='o')
...
def plot_searched_points_3d(results, solution, filename, min_recall=0.95, tuning_budget=TUNING_BUDGET):
...
def plot_efS_3d(results, solution, filename, min_recall=0.95, tuning_budget=TUNING_BUDGET):
...
def get_optimal_hyperparameter(results, min_recall=0.95):
    optimal_hyperparameters = None
    max_qps = 0.0
    for (M, efC, efS), (tuning_time, recall, qps, total_time, build_time, index_size) in results:
        if recall >= min_recall and qps > max_qps:
            max_qps = qps
            optimal_hyperparameters = ((M, efC, efS), 
                                       (round(tuning_time.item(),2), round(recall.item(), 3), round(qps.item(), 2),
                                        round(total_time.item(), 2), round(build_time.item(), 2), int(index_size.item())))
    if optimal_hyperparameters is None:
        print("No hyperparameters found with recall >= {min_recall}")
    return optimal_hyperparameters

def get_local_optimal_hyperparameter(results, min_recall=0.95):
    ...
    for cur_M in Ms:
        ...
        local_opt = None
        for (M, efC, efS), (tuning_time, recall, qps, total_time, build_time, index_size) in results:
            if M == cur_M and qps > max_qps and recall >= min_recall:
                max_qps = qps
                local_opt = ((M, efC, efS), 
                            (round(tuning_time.item(), 2), round(recall.item(), 3), round(qps.item(), 2),
                            round(total_time.item(), 2), round(build_time.item(), 2), int(index_size.item())))
    ...
    if not get_local_optimal_hyperparameters:
        print(f"No hyperparameters found with recall >= {min_recall}")
