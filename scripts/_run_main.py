...
def worker_function(params):
    ...
    # try:
    results = solution_func(impl=impl, dataset=dataset, recall_min=recall_min, qps_min=qps_min, sampling_count=sampling_count)
    return {
...
if __name__ == "__main__":
    ...
    all_combinations = list(itertools.product(IMPLS, DATASETS, SOLUTIONS, RECALL_MINS, [None], SAMPLING_COUNT))
    all_combinations += list(itertools.product(IMPLS, DATASETS, SOLUTIONS, [None], QPS_MINS, SAMPLING_COUNT))
    tasks = [
